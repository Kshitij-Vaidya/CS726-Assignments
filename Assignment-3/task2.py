...
if __name__ == "__main__":
    ...
    # Changed default max_output_len from 50 to 500.
    parser.add_argument("--max-output-len","-mol", type=int, required=False, default=500, help="Maximum number of new tokens to be generated.")
    ...
    # Pass the tokenizer so that the generator can decode tokens for debug prints.
    ...
    # Prevent division by zero error in RTF calculation.
    rtf = total_time / total_generated_tokens if total_generated_tokens > 0 else float('inf')
    
    print(f"""BLEU: {bleu.compute(predictions=generated_texts, references=reference_texts, max_order=1)['bleu']}
ROUGE-1: {float(rouge.compute(predictions=generated_texts, references=reference_texts)['rouge1'])}
ROUGE-2: {float(rouge.compute(predictions=generated_texts, references=reference_texts)['rouge2'])}
ROUGE-LCS: {float(rouge.compute(predictions=generated_texts, references=reference_texts)['rougeL'])}
RTF: {rtf}""")
